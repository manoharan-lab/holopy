...
C   For some compilers, ACOS must be raplaced by DARCOS and ASIN     
C   by DARSIN.                                                         
...
C   ANY DAMAGES THAT MAY RESULT FROM THE USE OF THE PROGRAM. 
     ...
     &     IT21(NPN6,NPN4,NPN4),IT22(NPN6,NPN4,NPN4)
      COMPLEX(KIND=16) S11,S12,S21,S22
      INTEGER NP,NDGS
...
C  COMPUTATION OF THE AMPLITUDE MATRIX
      CALL AMPL (NMAX,LAM,THET0,THET,PHI0,PHI,ALPHA,BETA,
     &           S11,S12,S21,S22)

      ITIME=MCLOCK()
      TIME=FLOAT(ITIME)/6000D0
      PRINT 1001,TIME
...
C   CALCULATION OF THE AMPLITUDE MATRIX          
      ...
      SUBROUTINE AMPL (NMAX,LAM,TL,TL1,PL,PL1,ALPHA,BETA,
     &                 VV,VH,HV,HH)  
      INCLUDE 'amplq.par.f'
      IMPLICIT REAL(KIND=8) (A-B,D-H,O-Z), COMPLEX(KIND=16) (C)
      REAL(KIND=8) AL(3,2), AL1(3,2), AP(2,3), AP1(2,3), B(3,3),
     &             R(2,2), R1(2,2), C(3,2), CA, CB, CT, CP, CTP, CPP, 
     &             CT1, CP1, CTP1, CPP1
     ...
     &     TI21(NPN6,NPN4,NPN4),TI22(NPN6,NPN4,NPN4)
      COMPLEX(KIND=16) CAL(NPN4,NPN4), VV, VH, HV, HH
      INTEGER NMAX
...
C____________COMPUTE MATRICES R AND R^(-1), EQ. (13)
      ...
      DO 500 M=0,NMAX
         ...
         DO 400 NN=NMIN,NMAX
            ...
            DO 400 N=NMIN,NMAX
               ...
               CT11=CMPLX(TR11(M1,N,NN),TI11(M1,N,NN))
               CT22=CMPLX(TR22(M1,N,NN),TI22(M1,N,NN))
               ...
               IF (M.EQ.0) THEN
                 ...
                 ELSE
                  ...
                  CT12=CMPLX(TR12(M1,N,NN),TI12(M1,N,NN))
                  CT21=CMPLX(TR21(M1,N,NN),TI21(M1,N,NN))
  ...
  500 CONTINUE
      ...
      PRINT 1005, TL,TL1,PL,PL1,ALPHA,BETA 
      PRINT 1006
      PRINT 1101, VV
      PRINT 1102, VH
      PRINT 1103, HV
...
c         PRINT 5000, CHECK
   ...
   20         PC=X*PB+(X*PB-PA)*(DJ-A)/DJ
          ...
          W(I)=W(M)
  100 CONTINUE
C5000 FORMAT ('QGAUSS DOES NOT CONVERGE, CHECK=',Q10.3)
...
C**********************************************************************
     ...
     *       A(NPN2,NPN2),C(NPN2,NPN2),D(NPN2,NPN2),E(NPN2,NPN2)
      REAL(KIND=8) TR1(NPN2,NPN2),TI1(NPN2,NPN2)
      COMPLEX(KIND=16) ZQ(NPN2,NPN2),ZW(NPN2)
...
C*****************************************************************
      ...
      IF (D.GE.1) GO TO 10
      E=SQRT(1D0-D*D)
      R=0.5D0*(D**(2D0/3D0) + D**(-1D0/3D0)*ASIN(E)/E)
      R=SQRT(R)
      RAT=1D0/R
      RETURN
   10 E=SQRT(1D0-1D0/(D*D))
      R=0.25D0*(2D0*D**(2D0/3D0) + D**(-4D0/3D0)*LOG((1D0+E)/(1D0-E))
     &   /E)
      R=SQRT(R)
      RAT=1D0/R
      RETURN

...
def viewcluster(cluster):
    ...
    l=pyplot.plot(cluster.centers[:,0]-cluster.centers[:,0].mean(),cluster.centers[:,1]-cluster.centers[:,1].mean(),'ro')
    pyplot.setp(l, 'markersize', 60)
    pyplot.xlim(-dist.max(),dist.max())
    pyplot.ylim(-dist.max(),dist.max())
    pyplot.subplot(1,3,2)
    l=pyplot.plot(cluster.centers[:,0]-cluster.centers[:,0].mean(),cluster.centers[:,2]-cluster.centers[:,2].mean(),'ro')
    pyplot.setp(l, 'markersize', 60)
    ...
    l=pyplot.plot(cluster.centers[:,1]-cluster.centers[:,1].mean(),cluster.centers[:,2]-cluster.centers[:,2].mean(),'ro')
    pyplot.setp(l, 'markersize', 60)
    pyplot.xlim(min(pyplot.xlim()[0],pyplot.ylim()[0]),max(pyplot.xlim()[1],pyplot.ylim()[1]))
    pyplot.ylim(min(pyplot.xlim()[0],pyplot.ylim()[0]),max(pyplot.xlim()[1],pyplot.ylim()[1]))
    pyplot.xlim(-dist.max(),dist.max())

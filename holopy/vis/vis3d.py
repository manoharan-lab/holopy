...
def show_sphere_cluster(s,color):
    ...
    if color=='rainbow':
        for i in arange(0,len(s.x)):
            print cm.gist_rainbow(float(i)/len(s.x))[0:3]
            numberofcolors=max(10,len(s.x))
            mlab.points3d(s.x[i], s.y[i], s.z[i], s.r[i], scale_factor=2.0, resolution=32, color=cm.gist_rainbow(float(i)/numberofcolors)[0:3])
